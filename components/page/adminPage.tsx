...
const AdminPage = ({ onLogout }: { onLogout: () => void }) => {
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        if (formData.is_staff) {
          ...
          setSiswaList((prev) =>
            prev.map((s) => (s.id === updateTarget ? updatedUser : s))
          );
        }
        setShowFormModal(false);
        setUpdateTarget(0);
  ...
  return (
    <div className="relative h-screen w-full overflow-hidden bg-gradient-to-br from-sky-50 via-white to-indigo-100 px-4 py-5 md:px-8 md:py-8 flex flex-col">
      ...
      <main className="relative mx-auto w-full max-w-6xl flex-1 flex flex-col min-h-0">
        {/* ===== HEADER — STICKY ===== */}
        <header className="relative z-50 mb-4 flex flex-col gap-4 rounded-[2rem] border-[.5px] border-cyan-500 bg-white/80 p-5 shadow-xl shadow-slate-200/50 backdrop-blur md:flex-row md:items-center md:justify-between shrink-0">
          ...
          <div className="relative flex items-center justify-between gap-3 md:justify-end">
            ...
            {showDropdown && (
              <>
                ...
                <div className="absolute right-0 top-[60px] z-90 w-64 overflow-hidden rounded-3xl border-[.5px] border-blue-500 bg-white shadow-2xl shadow-slate-200">
                  <div className="border-b border-slate-100 p-4">
                    <p className="text-sm font-bold text-slate-800">{me.nama_lengkap || 'Admin'}</p>
        ...
        {/* ===== STATISTIK CARDS — STICKY ===== */}
        <section className="mb-4 grid gap-4 md:grid-cols-3 shrink-0">
          <div className="rounded-[2rem] border-[.5px] border-cyan-500 bg-white/85 px-6 py-4 shadow-xl shadow-slate-200/50 backdrop-blur transition hover:-translate-y-1 hover:shadow-2xl">
        ...
        {/* ===== TABLE SISWA — SCROLLABLE ===== */}
        <section className="flex-1 flex flex-col min-h-0 overflow-hidden rounded-[2rem] border-[.5px] border-cyan-500 bg-white/90 shadow-2xl shadow-slate-200/60 backdrop-blur">
          {/* Header Table — Sticky */}
          <div className="border-b border-slate-100 p-6 shrink-0">
            <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              ...
              <button
                ...
                className="inline-flex items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-blue-500 to-indigo-600 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-blue-200 transition hover:-translate-y-0.5 hover:shadow-xl"
              >
                <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {/* Footer Table — Sticky Bottom */}
          <div className="flex flex-col gap-2 border-t border-slate-100 px-8 py-3 text-xs text-slate-400 md:flex-row md:items-center md:justify-between shrink-0">
      ...
      {/* ===== MODALS ===== */}
      {/* ... (semua modal tetap sama seperti kode kamu) ... */}
      ...
      {selected && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative z-[101] flex max-h-[90vh] w-full max-w-6xl flex-col overflow-hidden rounded-[2rem] bg-white shadow-2xl shadow-slate-900/20">
            ...
            <div className="grid flex-1 overflow-hidden lg:grid-cols-[1fr_330px]">
                ...
                {selectedHasilList.length > 0 ? (
                  <div className="space-y-4">
                    {selectedHasilList.map((hasil: any, index: number) => (
                      <div key={hasil.id || index} className="rounded-[1.7rem] border border-slate-100 bg-white p-5 shadow-sm">
                        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                          <div className="flex items-start gap-4">
                            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-blue-500 to-indigo-600 text-sm font-extrabold text-white">
                              {index + 1}
                            </div>
                            <div>
                          ...
                          <div className="grid grid-cols-3 gap-2 min-w-[300px]">
                            <div className="rounded-2xl border border-blue-100 bg-blue-50 p-3">
                              <p className="text-[10px] font-bold text-blue-500">Akademik</p>
                              <p className="mt-1 text-sm font-bold text-blue-800">{hasil.rekomendasi_akademik || '-'}</p>
                            </div>
                            <div className="rounded-2xl border border-purple-100 bg-purple-50 p-3">
                              <p className="text-[10px] font-bold text-purple-500">RIASEC</p>
                              <p className="mt-1 text-sm font-bold text-purple-800">{hasil.rekomendasi_riasec || '-'}</p>
                            </div>
                            <div className="rounded-2xl border border-emerald-100 bg-emerald-50 p-3">
                              <p className="text-[10px] font-bold text-emerald-500">Bakat</p>
                              <p className="mt-1 text-sm font-bold text-emerald-800">{hasil.rekomendasi_bakat || '-'}</p>
                            </div>
                          </div>
                        </div>
                        <div className="mt-4 flex justify-end border-t border-slate-100 pt-4">
                          <button
                            onClick={() => { setConfirmDelete(true); setPickHapus(hasil.id); }}
                            className="rounded-xl bg-gradient-to-br from-red-500 to-[#ff1265] px-6 py-2 text-sm font-extrabold text-white shadow-lg transition hover:from-[#de004e] hover:to-[#de004e]"
                          >
                            Hapus
                          </button>
                        </div>
              ...
              <div className="border-t border-slate-100 bg-white p-6 lg:border-l lg:border-t-0">
                <div className="sticky top-6 text-center">
                  <div className="mx-auto mb-4 flex h-24 w-24 items-center justify-center rounded-[2rem] bg-gradient-to-br from-blue-500 to-indigo-600 text-3xl font-extrabold text-white shadow-xl">
                    {getInitial(selected.nama_lengkap)}
                  </div>
                  <h3 className="text-lg font-extrabold text-slate-900">{selected.nama_lengkap || '-'}</h3>
                  <p className="text-sm font-medium text-blue-600">Kelas {selected.kelas || '-'}</p>
                  <div className="mt-5 space-y-3 rounded-[1.7rem] bg-slate-50 p-4 text-left">
                    ...
                    <div className="rounded-2xl bg-white p-4">
                      ...
                      <p className="text-sm font-bold break-all">{selected.email || '-'}</p>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <div className="rounded-2xl bg-white p-4">
                        <p className="text-xs text-slate-400">Usia</p>
                        <p className="text-sm font-bold">{selected.usia || '-'}</p>
                      </div>
                      <div className="rounded-2xl bg-white p-4">
      ...
      {showProfile && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative z-10 max-h-[90vh] w-full max-w-sm overflow-y-auto rounded-[2rem] bg-white p-6 shadow-2xl">
            <div className="text-center">
              <button onClick={() => setShowProfile(false)} className="absolute right-4 top-4 flex h-9 w-9 items-center justify-center rounded-full bg-slate-100 text-slate-500 transition hover:bg-slate-200">
                <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                </svg>
              </button>
              <div className="mx-auto mb-4 flex h-20 w-20 items-center justify-center rounded-[1.7rem] bg-gradient-to-br from-blue-500 to-indigo-600 text-2xl font-bold text-white shadow-lg">
                {getInitial(me.nama_lengkap)}
              </div>
              <h2 className="text-xl font-bold text-slate-900">{me.nama_lengkap || 'Admin'}</h2>
              ...
              <div className="space-y-3 rounded-3xl bg-slate-50 p-4 text-left">
                <div className="rounded-2xl bg-white p-3">
                  ...
                  <p className="text-sm font-semibold">{me.nip || '-'}</p>
                </div>
                <div className="rounded-2xl bg-white p-3">
                  <p className="text-xs text-slate-400">Email</p>
              ...
              <button onClick={() => { setShowProfile(false); handleOpenEdit(me); }} className="mt-5 w-full rounded-2xl bg-gradient-to-r from-yellow-500 to-orange-600 py-3 text-sm font-semibold text-white shadow-lg transition hover:-translate-y-0.5">
                Edit Profile
              </button>
              <button onClick={() => setShowProfile(false)} className="mt-2 w-full rounded-2xl bg-gradient-to-r from-blue-500 to-indigo-600 py-3 text-sm font-semibold text-white shadow-lg transition hover:-translate-y-0.5">
                Tutup
              </button>
            </div>
          </div>
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-[101] flex items-center justify-center p-4">
          ...
          <div className="relative z-10 max-h-[90vh] w-full max-w-sm rounded-[2rem] bg-white p-6 shadow-2xl text-center">
            ...
            <div className="mt-6 flex gap-3">
              <button onClick={() => setConfirmDelete(false)} className="flex-1 rounded-2xl bg-slate-100 px-4 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-200">Batal</button>
              <button onClick={() => handleDeleteHasilTes(pickHapus)} disabled={isLoading} className="flex-1 rounded-2xl bg-red-500 px-4 py-3 text-sm font-semibold text-white shadow-lg transition hover:bg-red-600 disabled:opacity-60">
      ...
      {showLogout && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative z-10 max-h-[90vh] w-full max-w-sm rounded-[2rem] bg-white p-6 shadow-2xl text-center">
            ...
            <div className="mt-6 flex gap-3">
              <button onClick={() => setShowLogout(false)} className="flex-1 rounded-2xl bg-slate-100 px-4 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-200">Batal</button>
              <button onClick={handleLogout} className="flex-1 rounded-2xl bg-red-500 px-4 py-3 text-sm font-semibold text-white shadow-lg transition hover:bg-red-600">Ya, Logout</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showFormModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative z-10 max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-[2rem] bg-white p-10 shadow-2xl">
            <button onClick={() => setShowFormModal(false)} className="absolute right-4 top-4 flex h-9 w-9 items-center justify-center rounded-full bg-slate-100 text-slate-500 transition hover:bg-slate-200">
              <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
              </svg>
            </button>
            <div className="mb-6">
              <p className="text-xs font-semibold uppercase tracking-[0.25em] text-blue-500">{formMode === 'tambah' ? 'Tambah Data' : 'Edit Data'}</p>
              <h2 className="mt-1 text-xl font-bold text-slate-900">{formMode === 'tambah' ? 'Tambah Siswa Baru' : (formData.is_staff ? 'Edit Data Admin' : 'Edit Data Siswa')}</h2>
              <p className="text-sm text-slate-500">{formMode === 'tambah' ? 'Lengkapi data akun dan identitas siswa.' : (formData.is_staff ? 'Perbarui data admin.' : 'Perbarui data siswa.')}</p>
            </div>
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                <label className="mb-1.5 block text-xs font-semibold text-slate-600">Nama Lengkap</label>
                <input type="text" value={formData.nama_lengkap} onChange={(e) => handleChange('nama_lengkap', e.target.value)} placeholder="Nama lengkap" className={`w-full rounded-2xl border px-4 py-3 text-sm text-slate-700 outline-none transition focus:ring-4 ${formErrors.nama_lengkap ? 'border-red-200 bg-red-50 focus:ring-red-100' : 'border-slate-200 bg-slate-50 focus:border-blue-200 focus:bg-white focus:ring-blue-100'}`} />
                {formErrors.nama_lengkap && <p className="mt-1 text-xs text-red-500">{formErrors.nama_lengkap}</p>}
              </div>
              {formData.is_staff ? (
                <div>
                  <label className="mb-1.5 block text-xs font-semibold text-slate-600">NIP</label>
                  <input type="text" value={formData.nip} onChange={(e) => handleChange('nip', e.target.value)} placeholder="Contoh: 198501012010011001" className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700 outline-none transition focus:border-blue-200 focus:bg-white focus:ring-4 focus:ring-blue-100" />
                </div>
              ) : (
                <div>
                  <label className="mb-1.5 block text-xs font-semibold text-slate-600">Kelas</label>
                  <input type="text" value={formData.kelas} onChange={(e) => handleChange('kelas', e.target.value)} placeholder="Contoh: 9-A" className={`w-full rounded-2xl border px-4 py-3 text-sm text-slate-700 outline-none transition focus:ring-4 ${formErrors.kelas ? 'border-red-200 bg-red-50 focus:ring-red-100' : 'border-slate-200 bg-slate-50 focus:border-blue-200 focus:bg-white focus:ring-blue-100'}`} />
                  {formErrors.kelas && <p className="mt-1 text-xs text-red-500">{formErrors.kelas}</p>}
                </div>
              )}
              <div>
                <label className="mb-1.5 block text-xs font-semibold text-slate-600">Usia</label>
                <input type="number" value={formData.usia} onChange={(e) => handleChange('usia', e.target.value === '' ? '' : Number(e.target.value))} placeholder="Contoh: 14" className={`w-full rounded-2xl border px-4 py-3 text-sm text-slate-700 outline-none transition focus:ring-4 ${formErrors.usia ? 'border-red-200 bg-red-50 focus:ring-red-100' : 'border-slate-200 bg-slate-50 focus:border-blue-200 focus:bg-white focus:ring-blue-100'}`} />
                {formErrors.usia && <p className="mt-1 text-xs text-red-500">{formErrors.usia}</p>}
              </div>
              <div>
                ...
                <div className="grid grid-cols-2 gap-2 rounded-2xl border border-slate-200 bg-slate-50 p-1.5">
                  <button type="button" onClick={() => handleChange('kelamin', 'pria')} className={`rounded-xl py-2.5 text-sm font-semibold transition ${formData.kelamin === 'pria' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>Laki-laki</button>
                  <button type="button" onClick={() => handleChange('kelamin', 'wanita')} className={`rounded-xl py-2.5 text-sm font-semibold transition ${formData.kelamin === 'wanita' ? 'bg-white text-pink-500 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>Perempuan</button>
                </div>
              </div>
              <div>
                <label className="mb-1.5 block text-xs font-semibold text-slate-600">Username</label>
                <input type="text" value={formData.username} onChange={(e) => handleChange('username', e.target.value)} placeholder="Username" className={`w-full rounded-2xl border px-4 py-3 text-sm text-slate-700 outline-none transition focus:ring-4 ${formErrors.username ? 'border-red-200 bg-red-50 focus:ring-red-100' : 'border-slate-200 bg-slate-50 focus:border-blue-200 focus:bg-white focus:ring-blue-100'}`} />
                {formErrors.username && <p className="mt-1 text-xs text-red-500">{formErrors.username}</p>}
              </div>
              <div>
                <label className="mb-1.5 block text-xs font-semibold text-slate-600">Email</label>
                <input type="email" value={formData.email} onChange={(e) => handleChange('email', e.target.value)} placeholder="email@sekolah.sch.id" className={`w-full rounded-2xl border px-4 py-3 text-sm text-slate-700 outline-none transition focus:ring-4 ${formErrors.email ? 'border-red-200 bg-red-50 focus:ring-red-100' : 'border-slate-200 bg-slate-50 focus:border-blue-200 focus:bg-white focus:ring-blue-100'}`} />
              ...
              {formMode === 'tambah' && (
                <>
                  <div>
                    ...
                    <div className="relative">
                      <input type={showPassword ? 'text' : 'password'} value={formData.password} onChange={(e) => handleChange('password', e.target.value)} placeholder="••••••••" className={`w-full rounded-2xl border py-3 pl-4 pr-16 text-sm text-slate-700 outline-none transition focus:ring-4 ${formErrors.password ? 'border-red-200 bg-red-50 focus:ring-red-100' : 'border-slate-200 bg-slate-50 focus:border-blue-200 focus:bg-white focus:ring-blue-100'}`} />
                      <button type="button" onClick={() => setShowPassword((prev) => !prev)} className="absolute right-3 top-1/2 -translate-y-1/2 rounded-xl px-2 py-1 text-xs font-semibold text-blue-600 hover:bg-blue-50">{showPassword ? 'Sembunyi' : 'Lihat'}</button>
                    </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input type={showConfirmPassword ? 'text' : 'password'} value={formData.confirmPassword} onChange={(e) => handleChange('confirmPassword', e.target.value)} placeholder="••••••••" className={`w-full rounded-2xl border py-3 pl-4 pr-16 text-sm text-slate-700 outline-none transition focus:ring-4 ${formErrors.confirmPassword ? 'border-red-200 bg-red-50 focus:ring-red-100' : 'border-slate-200 bg-slate-50 focus:border-blue-200 focus:bg-white focus:ring-blue-100'}`} />
                      <button type="button" onClick={() => setShowConfirmPassword((prev) => !prev)} className="absolute right-3 top-1/2 -translate-y-1/2 rounded-xl px-2 py-1 text-xs font-semibold text-blue-600 hover:bg-blue-50">{showConfirmPassword ? 'Sembunyi' : 'Lihat'}</button>
                    </div>
            ...
            <div className="mt-6 flex flex-col-reverse gap-3 md:flex-row">
              <button onClick={() => setShowFormModal(false)} disabled={isLoading} className="flex-1 rounded-2xl bg-slate-100 px-4 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-200 disabled:opacity-60">Batal</button>
              <button onClick={formMode === 'tambah' ? handleCreate : handleUpdate} disabled={isLoading} className="flex-1 rounded-2xl bg-gradient-to-r from-blue-500 to-indigo-600 px-4 py-3 text-sm font-semibold text-white shadow-lg transition hover:-translate-y-0.5 disabled:opacity-60">
      ...
      {showDeleteModal && deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative z-10 max-h-[90vh] w-full max-w-sm rounded-[2rem] bg-white p-6 shadow-2xl text-center">
            ...
            <p className="mt-2 text-sm text-slate-500">Apakah kamu yakin ingin menghapus <span className="font-semibold text-slate-800">{deleteTarget.nama_lengkap}</span>?</p>
            <p className="mt-1 text-xs text-red-500">Data yang sudah dihapus tidak bisa dikembalikan.</p>
            <div className="mt-6 flex gap-3">
              <button onClick={() => setShowDeleteModal(false)} disabled={isLoading} className="flex-1 rounded-2xl bg-slate-100 px-4 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-200 disabled:opacity-60">Batal</button>
              <button onClick={handleDelete} disabled={isLoading} className="flex-1 rounded-2xl bg-red-500 px-4 py-3 text-sm font-semibold text-white shadow-lg transition hover:bg-red-600 disabled:opacity-60">
